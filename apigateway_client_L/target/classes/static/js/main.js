...
var letters = [ 'f6c9cc', 'a8c0c0', 'FEBF36', 'FF7238', '6475A0', 'acc7bf',
		'5e5f67', 'c37070', 'eae160', 'bf7aa3', 'd7d967' ];
...
$('#user_alarm_text').click(function() {
	...
	$('#notice_popup').show(0, function() {
		getAlarm();
	});
});

$('#user_profile_text').click(function() {
	...
	$('#user_popup').show(0, function() {
		var userName = localStorage.getItem('name');
		var userEmail = localStorage.getItem('email');

		$('#editnameform').val(userName);
		$('#editemailform').val(userEmail);
...
function add_notice(notice) {
	var newPHtml = "<i class=\"w3-margin-right w3-large w3-text-teal\"></i> "
			+ "보낸 사람 &emsp; : &emsp;" + notice.sender_id
			+ " <br> 제목 &emsp; : &emsp;" + notice.article_id;
	var para = document.createElement("P");
	para.id = "notice_text";
	// console.log(para.id);
	para.innerHTML = newPHtml;
	// console.log(para.innerHTML);
	var hr = document.createElement("HR");
	document.getElementById("notice_box").appendChild(para);
	document.getElementById("notice_box").appendChild(hr);
}

function add_story(story) {
	var newPHtml = story.message;
	var color = "#";
	var para = document.createElement("div");
	var para_o = document.createElement("div");
	para.className = "story_circle";
	para_o.className = "story_owner";
	para.id = story.story_id;
	para_o.id = para.id;

	color += letters[para.id % 11];
	para.style.background = color;
	para.innerHTML = newPHtml;
	// console.log(para.innerHTML);
	document.getElementById("lastlogo").appendChild(para);
	document.getElementById("lastlogo").appendChild(para_o);
}

function add_MyStory(story) {

	var newPHtml = "<i class=\"fa fa-home fa-fw w3-margin-right w3-large w3-text-teal\"></i> "
			+ notice.message;
	var para = document.createElement("P");
	para.id = "notice_text";
	// console.log(para.id);
...
function greetingPageAgain() {
	...
	$("#logo_greeting").fadeIn(0, function() {
		...
		$("#centerbox, #avatarcontainer").show(0, function() {
			...
			if (name) {
				$("#main_id").html(localStorage.getItem('username'));
				$("#main_name").html(localStorage.getItem('name'));
				$("#main_email").html(localStorage.getItem('email'));
